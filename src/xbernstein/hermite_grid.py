...
from .hermite import _multi_indices, hermite_interpolate_1d, hermite_interpolate_2d
from .hermite import hermite_interpolate_3d, hermite_interpolate_4d
...
class _HermiteGrid(eqx.Module):
    ...
    def _initialize(self, axes, groups, dimension):
        ...
        expected_counts = tuple(len(_multi_indices(dimension, 2, total)) for total in range(1, dimension + 1))
        ...
        converted = []
        for name, group in zip(("f",) + tuple(f"d{i}" for i in range(1, dimension + 1)), values):
            group = group.astype(dtype)
            group = eqx.error_if(group, jnp.any(~jnp.isfinite(group)), f"{name} must contain only finite values")
    ...
    def _local_coordinates(self, point, cell_index):
        ...
        ends = jnp.stack([axis[index + 1] for axis, index in zip(self.axes, cell_index)])
    ...
    def _cell_vertices(self, group, cell_index):
        ...
        for offset in range(self.dimension):
            result = jnp.take(result, cell_index[offset] + jnp.arange(2), axis=len(self.shape) + offset)
        return result

    def cell_interpolant(self, cell_index) -> Shaped:
        cell_index = jnp.asarray(cell_index)
        if cell_index.shape != (self.dimension,):
            raise ValueError(f"cell_index must have shape ({self.dimension},), got {cell_index.shape}")
        cell_index = cell_index.astype(jnp.int32)
        maximum = jnp.asarray(self.grid_shape, dtype=jnp.int32) - 2
        cell_index = eqx.error_if(cell_index, jnp.any(cell_index < 0) | jnp.any(cell_index > maximum), "cell_index is outside the grid")
        widths = jnp.stack([axis[index + 1] - axis[index] for axis, index in zip(self.axes, cell_index)])
        ...
        for total, source in enumerate(self.groups[1:], start=1):
            ...
            alphas = _multi_indices(self.dimension, 2, total)
            scales = jnp.stack([jnp.prod(widths ** jnp.asarray(alpha)) for alpha in alphas])
            if len(alphas) == 1:
                group = group * scales[0]
            else:
                group = group * scales.reshape((1,) * (group.ndim - 1) + (len(alphas),))
            local_groups.append(group)
        interpolators = (hermite_interpolate_1d, hermite_interpolate_2d, hermite_interpolate_3d, hermite_interpolate_4d)
    ...
    def _split_segment(self, start, end):
        ...
        for axis_index, axis in enumerate(self.axes):
            ...
            parameters = jnp.where(denominator != 0.0, (axis[1:-1] - start[axis_index]) / denominator, jnp.inf)
            candidates.append(jnp.where((parameters > 0.0) & (parameters < 1.0), parameters, jnp.inf))
        ...
        valid = jnp.isfinite(interval_starts) & jnp.isfinite(interval_ends) & (interval_ends - interval_starts > tolerance)
        ...
        def add(i, state):
            ...
            def write(s):
                ...
                return out + 1, cells.at[out].set(cell), endpoints.at[out].set(local), mask.at[out].set(True)
            return jax.lax.cond(valid[i] & (out < capacity), write, lambda s: s, (out, cells, endpoints, mask))

        _, cells, endpoints, mask = jax.lax.fori_loop(0, interval_starts.shape[0], add, (jnp.array(0, dtype=jnp.int32), cells, endpoints, mask))
        ...
        endpoints = endpoints.at[0].set(jnp.where(no_piece, jnp.stack((local, local)), endpoints[0]))
