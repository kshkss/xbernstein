...
class Bernstein3D(_TensorBernstein):
    ...
    With coefficient array shape ``(*batch, n_0 + 1, n_1 + 1, n_2 + 1)``, the
    final three axes store $c_{i_0,i_1,i_2}$ and the polynomial is

    $$
    p(\mathbf{u})=
    \sum_{i_0=0}^{n_0}\sum_{i_1=0}^{n_1}\sum_{i_2=0}^{n_2}
    c_{i_0,i_1,i_2}
